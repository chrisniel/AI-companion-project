...
> **Status:** Active Working Manifest (Pass R10 Baseline)  
...
> **Purpose:** Prevents informal, unreviewed, or deprecated ideas from drafts, historical archives, or recovery notes from silently becoming canonical architecture without explicit review, attribution, release allocation, and canonical destination.
...
- **`REJECTED`**: Explicitly excluded from intended product architecture.
...
| `SYSTEM_BASELINE.md` §2 | **Memory Persistence (FTS5)** | Implemented (V1) | `LOCKED` | `VERIFIED` | `PC V1` | `docs/04_Architecture/01_Domains/memory-and-personalization.md` | `PRESERVE` | Verified SQLite WAL + FTS5 lexical keyword retrieval baseline. Scoped under Profile ownership (Decision D7). |
| `SYSTEM_BASELINE.md` §2 | **Tasks Lifecycle Foundation** | In Progress (V1) | `APPROVED` | `IN PROGRESS` | `PC V1` | `docs/04_Architecture/01_Domains/tasks-reminders-alarms-and-routines.md` | `CONSOLIDATE` | Task CRUD, soft-delete, and retention thresholds implemented. Disambiguated from Reminders, Alarms, and Routines under Decision D10. |
...
| `ANDROID_COMPANION_ARCHITECTURE.md` §3 | **Android Device-Local TTS** | Post-V1 Optional | `PROVISIONAL` | `NOT STARTED` | `ANDROID LATER` | `docs/04_Architecture/01_Domains/android-companion.md` | `KEEP OPEN` | Historical phrasing "where feasible" confirmed non-blocking. Device-local TTS is NOT a locked Android V1 requirement. |
