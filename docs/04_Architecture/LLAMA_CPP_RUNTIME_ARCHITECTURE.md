...
- **In-Process Fallback Nuance:** `_resolve_model_path()` in `llama_cpp.py` resolves requested filenames against `settings.MODELS_DIR` (`<REPO_ROOT>/models/`), but this function serves the direct in-process `llama-cpp-python` binding fallback and legacy test paths, rather than the active managed-router model selection.
